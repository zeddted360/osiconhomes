...
const ServicesSection: React.FC = () => {
  ...
  const ServiceCard: React.FC<ServiceCardProps> = React.memo(({ service }) => {
    ...
    // Truncate description to first 100 characters or first sentence
    const truncateDescription = (
      text: string,
      maxLength: number = 100
    ): string => {
      if (text.length <= maxLength) return text;
      const sentenceEnd = text.indexOf(".", maxLength);
      if (sentenceEnd !== -1 && sentenceEnd < maxLength * 1.5) {
    ...
    return (
      <div id="service" className="group relative bg-white rounded-3xl shadow-lg hover:shadow-xl transition-all duration-500 overflow-hidden transform hover:-translate-y-2">
        {/* Image section */}
        ...
        <div className="p-6 space-y-4">
          ...
          <button
            aria-label={
              isExpanded
                ? `Read less about ${service.title}`
                : `Read more about ${service.title}`
            }
            aria-expanded={isExpanded}
            aria-controls={`description-${service.id}`}
            onClick={toggleDescription}
            className="group/btn relative flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-amber-400 to-orange-500 text-white font-semibold rounded-xl shadow-md hover:shadow-amber-500/30 transition-all duration-300 transform hover:scale-105 overflow-hidden"
          >
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12 space-y-20">
        ...
        <div className="space-y-12">
          <div className="flex items-center space-x-4">
            <h3 className="text-3xl font-bold text-gray-900">Construction</h3>
            <div className="flex-1 h-px bg-gradient-to-r from-gray-300 to-transparent"></div>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
